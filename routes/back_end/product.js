...
export default class extends controller {
  renders () {
    ...
    this.router.get('/admin/product_edit', this.api.isLogin(), async (ctx, next) => {
      ...
      if (series === 'pump') {
        let product = await this.DBModule.Pumps.findPump({
          _id: newsId
        }) // 获取荣誉资质总条数
        if (product.status) {
        ...
        let product = await this.DBModule.Seals.findSeal({
          _id: newsId
        }) // 获取荣誉资质总条数
        if (product.status) {
  ...
  actions () {
    ...
    this.router.post('/product/status', this.api.isLogin(), async (ctx, next) => {
      ...
      let changeStatus
      if (series === 'pump') {
        changeStatus = await this.DBModule.Pumps.changePumpStatus({
          _id: productId,
          hidden: status
        }) // 修改泵类产品状态
      } else {
        changeStatus = await this.DBModule.Seals.changeSealStatus({
          _id: productId,
          hidden: status
        }) // 修改密封产品状态
      }
    ...
    this.router.post('/product/delete', this.api.isLogin(), async (ctx, next) => {
      let productId = ctx.request.body.id
      let series = ctx.request.body.series
      let imgUrl = ctx.request.body.imgUrl // 产品图
      let imgStructUrl = ctx.request.body.imgStructUrl // 产品结构图
      let deleteProduct
      let deleteImg
      let deleteStructImg
      if (series === 'pump') {
        deleteProduct = await this.DBModule.Pumps.deletePump({
          _id: productId
        })
        deleteImg = await this.api.removeFiles(imgUrl)
      } else {
        deleteProduct = await this.DBModule.Seals.deleteSeal({
          _id: productId
        })
        deleteImg = await this.api.removeFiles(imgUrl)
        deleteStructImg = await this.api.removeFiles(imgStructUrl)
      }
