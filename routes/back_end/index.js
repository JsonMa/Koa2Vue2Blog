...
export default class extends controller {
  renders () {
    ...
    this.router.get('/admin/honor_detail', this.api.isLogin(), async (ctx, next) => {
      ...
      let honor = await this.DBModule.Honor.findHonor({
        _id: honorId
      }) // 获取荣誉资质总条数
      if (honor.status) {
    ...
    this.router.get('/admin/enterprise_detail', this.api.isLogin(), async (ctx, next) => {
      ...
      let pageNum = ctx.request.query.page || 1
      let enterprise = await this.DBModule.Enterprise.findEnterprise({
        _id: enterpriseId
      })
  ...
  actions () {
    ...
    this.router.post('/honor/status', this.api.isLogin(), async (ctx, next) => {
      ...
      let changeHonrStatus = await this.DBModule.Honor.changeHonrStatus({
        _id: honorId,
        hidden: status
      }) // 获取荣誉资质总条数
    ...
    this.router.post('/honor/delete', this.api.isLogin(), async (ctx, next) => {
      ...
      let deleteHonor = await this.DBModule.Honor.deleteHonor({
        _id: honorId
      }) // 获取荣誉资质总条数
    ...
    this.router.post('/enterprise/status', this.api.isLogin(), async (ctx, next) => {
      let honorId = ctx.request.body.id
      let status = ctx.request.body.status !== 'false'
      let changeEnterpriseStatus = await this.DBModule.Enterprise.changeEnterpriseStatus({
        _id: honorId,
        hidden: status
      }) // 获取荣誉资质总条数
    ...
    this.router.post('/enterprise/delete', this.api.isLogin(), async (ctx, next) => {
      ...
      var deleteEnterprise = await this.DBModule.Enterprise.deleteEnterprise({
        _id: enterpriseId
      }) // 获取荣誉资质总条数
